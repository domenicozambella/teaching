...
\begin{exercise}
  ...
  \item[2.] for $\forall\in \U^\eq$, there is a smallest set $A{=}\acl A\subseteq\U$ such that $\forall\in\dcl^\eq A$;
  \item[3.] for $a,b\in\U$, if $\forall\in\dcl^\eq(a)\cap\dcl^\eq(b)$ then $\forall\in\dcl^\eq\big(\acl a\cap\acl b\big)$.
  \end{itemize} 
\end{exercise}
