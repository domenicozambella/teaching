...
\begin{exercise}
Let $N$ consist of 3 copies of $\ZZ$ that share the same non negative part. The language has only a function symbol for the successor function (which will be non invertible in $0$).  Let $M$ be the substructure of $MN$ obtained erasing one copy of $\ZZ$. 
\begin{itemize}
\item[1.] Prove that $M\not\equiv N$. 
\item[2.] Suppose instead that $N$ is build from infinitely many copies of $\ZZ$, prove that $M\equiv N$. 
\item[3.] If $N$ is like in \ssf{2} is $M\preceq N$\,?
\end{itemize}
\end{exercise}
