...
\begin{itemize}
  \item[1.] Prove that $N$ is \textit{not\/} a random graph.
  \item[2.] Assuming $|N_1|=|N_2|=\omega$ prove that $N_1$ is not definable without parameters.\\
  Hint: definable sets are fixed by automorphisms.
  \item[3.] Show that the assumption $|N_1|=|N_2|=\omega$ is unnecessary\\
  Hint: use downward L\"owenheim-Skolem.
  \item[4.] Write a first order formula $\psi(x,y)$ true if $x$ and $y$ belong to the same connected component of $N$.
  \item[5.] Conclude that $N_1$ is definable (with parameters).
  \item[6.] Axiomatize (in words) the class of graphs that are free union of two random graphs.
\end{itemize}
\end{exercise}
\end{document}
