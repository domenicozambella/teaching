...
Se guardiamo in un momento qualsiasi troveremo il rospo in {\sf a}, {\sf b}, o {\sf c} con probabilità rispettivamente $\pyl{equilibrio[a]}$, $\pyl{equilibrio[b]}$, e $\pyl{equilibrio[c]}$. Supponiamo di osservare il rospo in {\sf a} al tempo $t=1$. 

\begin{itemize}
\item[1.] Qual è la probabilità che al tempo $t=2$ il rospo si trovi in {\sf b}~?
...
$\Pr(R_{t+1}={\sf a}~|~R_t={\sf b})\ =\ ${\color{blue}$\py{pba/10}$\hfill Risposta 1}

$\Pr(R_{t+1}={\sf c}~|~R_t={\sf a})\ =\ \py{pac/10}$
...
\ =\ ${\color{blue}$\py{pac*equilibrio[c]/(equilibrio[a]*10)}$\hfill Risposta 2}
