...
$\Pr\bigg(Z\le\py{z}+\dfrac{\varepsilon}{\sigma}\bigg) = \py{alpha}+\Pr(Z\le\py{z})$

$\py{z}+ \dfrac{\varepsilon}{\py{sigma}}$ = {\tt  qnorm( \py{alpha} + pnorm(\py{z}) ) }

$\varepsilon$ = {\color{blue}\tt  \py{sigma} * ( qnorm( \py{alpha} + pnorm(\py{z})\ ) + \py{-z} ) }
