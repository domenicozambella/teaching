...
\begin{exercise}\label{ex_heir_type}
  Show that for every $b\in\U^z$ there is a type $p(x\,;z)\subseteq L(M)$ such that  for every  $a\in\U^x$ and every $b'\in\U^z$

  \hfil$a\cnonfork_M b'\equiv_M b\ \ \IFF\ \ a,b'\models p(x\,;z).$
\end{exercise}
